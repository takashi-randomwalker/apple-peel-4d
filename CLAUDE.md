...
正多胞体では**根の安定化部分群が候補集合に推移的に作用する**ので，その群で不変な量はどれも候補間で同じ値になる。max w も回転角も Det も全てタイになる（apple-peel 側で xy クロス積という二次基準が必要だったのと同じ現象）。**等変な規則は原理的に決められない。** 解は「良い規則を探す」ではなく「主張から規則を消す」こと。
...
**持ち時間：質疑込み15分，交代1分で16分刻み** → 実質12分弱。本編9枚＋backup 2枚，16:9。
...
| 3 | Face-rotation BFS の3ステップ | 1:40 |
| 4 | Validity test と `RegionMember` の罠 | 1:20 |
| 5 | 主定理＋全6胞体のネット（`260612AllFaceRotation.pdf`） | 1:40 |
| 6 | apple-peel との対比 | 1:30 |
| 7 | **And more**：タイブレークの問いに答えた | 1:50 |
| 8 | **⚠ Warning**：一様は規則に敏感 | 1:40 |
| 9 | Open problems ＋ GitHub URL | 0:40 |
...
- **図の様式が混在**：Fig 1 は黒背景グレースケール，5胞体と grand antiprism は白背景カラー。`make_talk_figures.m` に黒版を追加して揃えるべき
- **`dove` カラーテーマだと alertblock の枠が出ない** — Warning の視覚的インパクトが弱い。テーマ選定時に要調整
- **Overfull vbox が3箇所**（スライド4・7・8）。文言を削るか行間を詰める必要あり
- テーマは未選定（ユーザーが後で決める）。最終的にファイル形式を変更する可能性あり
